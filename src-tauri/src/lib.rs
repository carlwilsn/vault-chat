...
// ----- Cross-machine sync daemon -----
//
// Exposes the active vault's conversations.jsonl via a small HTTP
// server. A client running on another machine pulls the snapshot, then
// long-polls for changes via /poll. POST /commit pushes the client's
// local edits back so both sides converge.
//
// This is intentionally minimalist: full-snapshot replication keeps the
// daemon code under 100 lines and lets the frontend treat the daemon as
// a remote-backed `readConversations` / `writeConversations`. Latency is
// fine for the personal-use scale (one user, two machines).
//
// The daemon also lets the frontend nominate a Tailscale hostname so the
// status row can show "home-box.tail-scale.ts.net" without us having to
// poke at the Tailscale CLI directly.
...
fn daemon_status() -> DaemonStatus {
    DaemonStatus {
        running: DAEMON_RUNNING.load(Ordering::SeqCst),
        clients: 0,
        listen: None,
        error: None,
    }
...
async fn daemon_start(
    ...
    {
        let mut slot = DAEMON_STOP.lock().unwrap();
        *slot = Some(stop_tx);
    }
...
fn daemon_stop() -> Result<(), String> {
    if let Some(tx) = DAEMON_STOP.lock().unwrap().take() {
        let _ = tx.send(());
    }
    DAEMON_RUNNING.store(false, Ordering::SeqCst);
    Ok(())
...
#[tauri::command]
async fn tailscale_hostname() -> Result<Option<String>, String> {
    // Probe for the Tailscale CLI. If installed and the device is
    // online, surface the MagicDNS name. Returns None when Tailscale
    // isn't installed or the device isn't on a tailnet.
    let result = tauri::async_runtime::spawn_blocking(|| -> Option<String> {
        #[cfg(windows)]
        let mut cmd = {
            use std::os::windows::process::CommandExt;
