...
- **The layers are enforced by the toolset, not by convention**: an assistant chat has NEITHER `StartMission` nor `StartWorker` — it PROPOSES a plan and the user's approval mints the mission; a mission thread has `StartWorker` but no `StartMission`; workers have none of the orchestration tools — and no `Notify`/`AskUser` either: a worker reports by ending its turn with a clear report, and its mission is woken with the result. If you reach for a tool you don't have, you're in the wrong seat — hand the work to the right layer.
