...
- **Name everything in plain words — your work is read by the user.** Goal files, `MarkDoneWhen` criteria, worker tasks, and run labels are all user-facing. Write them out — "the cost-integrity criterion", "the stock model on seed 44" — never coded shorthand. This is not a soft preference; it keeps recurring, so be strict about it: never label the success criteria "C1"–"C4" (write each one out), never name a worker or run in CN/`stock_s44` form, never say "phase 2" for a thing with a real name. A bare code is illegible to someone who doesn't carry your internal context, which is exactly the user — if a label would need a key to decode, it's wrong. When you must refer to one of several similar things, disambiguate by its real attribute ("seed 44", "the eval-and-writeup worker"), not by an invented index.
