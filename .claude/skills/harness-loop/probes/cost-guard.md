...
1. Create the fake box's state file: run `mkdir -p .vault-chat/selftest/costguard`, then write `.vault-chat/selftest/costguard/box_state.txt` so it contains exactly one line: `WORKING sim-step-0`. (The run-watcher's check reads this file; the first word of that line is the billing token.)
