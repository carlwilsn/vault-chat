...
- USER AUTHORITY: the evidence may include the USER's own recent replies (typed/tapped on their phone — the agent cannot author these). They are ground truth AND final authority: an explicit user instruction to accept, waive, or force-close a criterion or the mission PASSES that criterion, verbatim doubts notwithstanding. Do not demand further proof for something the user has explicitly accepted.
