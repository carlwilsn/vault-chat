...
export async function runAgent(params: {
  ...
  voiceMode?: boolean;
  // Supervisor role: the always-on orchestrator prompt (persistent mind, goal
  // loop, worker steering) used by mission threads.
  ...
  try {
    ...
    // Supervisor role: an always-on orchestrator applied to mission threads, but
    // NOT the interactive assistant (it gets the lighter assistant prompt below).
    // A vault that doesn't want a supervisor leaves the file empty and stays a
    // plain agent.
    const supervisorNote =
      supervisorMode && !assistantMode && vaultSupervisor.trim()
...
Be terse. If the task is research, return findings as a structured list with file:line citations. If the task is a code change, make the change and return what you did + the affected files.`;
    ...
    if (spec.provider === "anthropic") {
      // Opus 4.7+, Sonnet 5, and the Fable/Mythos 5 tier use the new adaptive
      ...
      // On these models the old { type:"enabled", budget_tokens } shape is
      // REJECTED with a 400 ("thinking.type.enabled is not supported … use
      // adaptive") — sonnet-5 is the chat-pane default AND the two-lane
      // conversational-front model, so omitting it here silently broke every
      // "talk it through" probe reply. Older models (Opus 4.6, Sonnet 4.6,
      // Haiku 4.5) still accept the enabled+budgetTokens shape, and sending
      // the new keys to them fails — so branch. (Fable/Mythos ship after 4.8
      // and inherit its adaptive API shape; revert this clause if their API
      // ever diverges.)
      const isAdaptive = /^claude-(opus-4-(7|8)|sonnet-5|fable-|mythos-)/i.test(spec.id);
